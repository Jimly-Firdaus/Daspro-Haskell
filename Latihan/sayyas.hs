-- DEFINISI DAN SPESIFIKASI LIST
    ...
    {-makeUnique yang menerima masukan sebuah list of character, misalnya lc, 
    dan menghasilkan list dengan elemen-elemen unik, yaitu, kemunculan setiap elemen 
    (jika tidak kosong), hanya 1-}
    ...
    makeUnique lc =
        if isEmpty lc then ""
        else if isOneElmt lc then konso (head lc) (makeUnique (tail lc))
        else if head(konso (head lc) []) /= (head lc) then [head lc] ++ konso (head lc) (makeUnique (tail lc))
        else makeUnique (tail lc)
    main = do
        print(makeUnique ['s','a','y','y','a','s'])
