module ListOfCharacter where
    ...
    inverse :: [Char] -> [Char]
    {-
    inverse lc menerima masukan sebuah list of character, misalnya lc, dan menghasilkan list, misalnya lc’, 
    yang berisi elemen-elemen lc dengan urutan yang dibalik. Jika lc = [e1,e2,...en-1,en] 
    maka lc’ = [en,en-1,...,e2,e1].
    -}
    --REALISASI
    inverse lc = 
        if isEmpty lc then []
        else if isOneElmt lc then [head lc]
