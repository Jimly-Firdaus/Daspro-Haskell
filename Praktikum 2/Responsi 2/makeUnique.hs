module ListOfCharacter where
    ...
    {-
    makeUnique l menerima masukan sebuah list of character, misalnya lc, dan 
    menghasilkan list dengan elemen-elemen unik, yaitu, kemunculan setiap elemen 
    (jika tidak kosong), hanya 1.
    -}
    --REALISASI
    makeUnique l = 
        if isEmpty l then ""
            else if isOneElmt l then l
                else if (head l) == head(konso (head l) []) then makeUnique(tail l)
                else konso (head l) (makeUnique(tail l))
    main = do
        print(makeUnique ['s','a','y','y','a','s','x'])